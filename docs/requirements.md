...
Bridge接続先はChrome拡張の設定画面からIPアドレス・URLを手入力する。

v1ではLAN内Bridgeへの認証は行わない。
...
Discordメッセージへホバーした際に表示される既存のアクション群に、独自の **Claudeボタン** を追加する。
...
定型アクションはChrome拡張へハードコードせず、**Bridge側の設定ファイル**で管理する。

各アクションは表示名とClaudeへ渡すプロンプトテンプレートを持つ。
...
返信チェーンは自動選択された状態で表示する。

### 5.4 長い返信チェーン

返信チェーンは原則すべて取得する。

ただし **20件以上** の場合は警告を表示する。
...
基本はDiscord Web上のDOMから取得する。

DOMだけでは必要なメッセージを取得できない場合、ブラウザ側で利用可能なDiscordのクライアントデータ/APIを利用して追加取得を試みる。
...
Claudeへ渡す各Discordメッセージには、可能な限り以下を含める。
...
これにより、ClaudeがJiraやGitHub Issueを作成するときに元Discord投稿への参照を記載できるようにする。
...
以下はBridge側へ一時取得し、Claude Codeから参照可能にする。
...
取得した添付ファイルはBridge側の一時ディレクトリへ保存する。
...
Discordから初めて送信した時点で、新しいClaude Codeセッションを開始する。
...
Claudeセッション継続中にDiscord側の会話が進んでも、自動では取り込まない。

Side Panelから明示的に **「Discordの最新コンテキストを追加」** を実行する。
...
Side Panelで現在Claudeセッションを開いている場合、別のDiscordメッセージのClaudeボタンから **「現在のセッションに追加」** を選択可能にする。
...
Claudeセッション一覧はChrome Side Panelを中心UIとする。
...
セッションタイトルは初回内容をもとにClaudeが短く自動生成する。

手動編集機能はv1では不要。

### 9.3 セッション保存期間

Chrome拡張独自のアーカイブ期間は設けない。

Claude Code側のセッション保存期間に追従する。

現在の利用環境では7日間だが、拡張側へ7日という固定値は持たせない。

Side Panelを開いたときにClaude Code側のセッション存在確認を行い、すでに存在しないセッションは一覧から自動除去する。

- Archived状態は作らない
- 手動削除UIは作らない

拡張側ではClaude Codeセッションを参照するための索引情報のみ保持する。
...
DiscordからClaudeへ送信した直後、Side Panelを自動で開き、そのセッションを表示する。

ユーザーは実行中でもDiscordの別チャンネルへ移動可能。

### 10.2 ストリーミング

Claude Codeの出力はリアルタイムにSide Panelへストリーミングする。

BridgeはClaude Codeのstreaming出力をChrome拡張へ転送する。
...
結果には **クリップボードへコピーするボタン** を設ける。

Discordへの自動投稿機能はv1では持たない。

---

## 11. 複数セッション

Claude Codeセッションは複数同時実行可能とする。

Bridge側では並列実行数を制限しない。

各セッションは完全に独立したClaude Codeプロセスとして扱う。

拡張アイコンには、完了した未確認セッション数をバッジ表示する。
...
実行中のClaude CodeセッションをSide Panelから停止できる。

停止は現在実行中の処理のみを中断する。

停止後もセッション自体は維持し、同じセッションへ追加指示を送信可能とする。
...
Repoを指定しない通常の処理は、専用WorkspaceでClaude Codeを起動する。
...
送信UIでは通常 **「一般」** を選択状態にする。

必要な場合のみプロジェクトSelectorを開いて対象Repoを指定する。

### 14.3 Project一覧

Bridge側でプロジェクト一覧を生成する。

設定されたProject Root配下を探索し、`.git` が存在するディレクトリのみProjectとして認識する。
...
調査セッションと実装セッションは両方保持する。

Side Panel上では通常の一覧として表示し、実装セッションから **元セッションへのリンク** を表示する。
...
Claude Codeセッションは、Side Panelからターミナル作業へ移行可能にする。
